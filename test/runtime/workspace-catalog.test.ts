...
describe("workspace teammate lifecycle", () => {
  ...
  it("keeps generated names unique without ignoring archived names or letter case", () => {
    const create = (id: string, name: string) =>
      catalog.createBot(
        id,
        { name, title: name, description: "A helpful teammate." },
        "Help the owner",
        "@cf/zai-org/glm-5.3-flash",
        2
      );

    expect(create("bot-1", "Avery")).toMatchObject({ name: "Avery", title: "Avery" });
    expect(catalog.archiveBot("bot-1")).toMatchObject({ hidden: true });
    expect(create("bot-2", "avery")).toMatchObject({ name: "avery 2", title: "avery 2" });
    expect(create("bot-3", "AVERY")).toMatchObject({ name: "AVERY 3", title: "AVERY 3" });
  });

  it("reads files only through their owning teammate", () => {
    for (const id of ["bot-1", "bot-2"]) {
      catalog.createBot(
        id,
        { name: id, title: id, description: "A helpful teammate." },
        "Help the owner",
        "@cf/zai-org/glm-5.3-flash",
        2
      );
    }
    const file = catalog.createFile({
      botId: "bot-1",
    ...
    expect(catalog.getFile(file.id, "bot-1")).toEqual(file);
    expect(catalog.getFile(file.id, "bot-2")).toBeNull();
    expect(catalog.getFile("missing", "bot-1")).toBeNull();
  });
  ...
  it("deletes teammate data and keeps overall usage anonymous", () => {
    ...
    tasks.startTask("task-1", "bot-1", "Research this");
    tasks.recordUsage({
      botId: "bot-1",
      estimatedUsd: 0.01,
      id: "usage-1",

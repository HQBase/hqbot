...
describe("workspace migrations", () => {
  ...
  it("builds a fresh workspace without the legacy connection table", () => {
    ...
    expect(
      database.prepare("SELECT name FROM pragma_table_info('bots') WHERE name = 'max_steps'").get()
    ).toEqual({ name: "max_steps" });
    expect(
      database
        .prepare(
          "SELECT name FROM pragma_table_info('tasks') WHERE name IN ('wake_at', 'work_state') ORDER BY name"
        )
        .all()
    ).toEqual([{ name: "wake_at" }, { name: "work_state" }]);
  });
  ...
  it("removes legacy mail content and keeps usage totals anonymous", () => {
    ...
    database.exec(`
      ...
      VALUES ('chat-task', 'bot-1', 'chat', 'completed', 'Keep this chat',
        '2026-08-30T12:00:00.000Z', '2026-08-30T12:00:00.000Z');
      INSERT INTO tasks (id, bot_id, source, status, prompt, created_at, updated_at)
      VALUES ('legacy-running', 'bot-1', 'chat', 'working', 'Old runtime work',
        '2026-08-30T12:00:00.000Z', '2026-08-30T12:00:00.000Z');
    ...
    expect(database.prepare("SELECT id, source, status FROM tasks ORDER BY id").all()).toEqual([
      { id: "chat-task", source: "chat", status: "completed" },
      { id: "legacy-running", source: "chat", status: "cancelled" }
    ]);
    ...
    expect(database.prepare("SELECT max_steps FROM bots WHERE id = 'bot-1'").get()).toEqual({
      max_steps: null
    });
    expect(
      database

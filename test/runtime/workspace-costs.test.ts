...
describe("workspace cost snapshot", () => {
  ...
  it("separates AI tokens and computer seconds for each scope", () => {
    ...
    expect(Object.keys(costs.services.overall).sort()).toEqual(["sandbox", "workersAi"]);
    expect(costs.overall.estimatedUsd).toBeCloseTo(0.046168);
    expect(costs.selectedBot.estimatedUsd).toBeCloseTo(0.016168);
    expect(costs.selectedTask.estimatedUsd).toBeCloseTo(0.016168);
    expect(costs.services.overall.workersAi).toMatchObject({
      inputUnits: 200,
      outputUnits: 50,
      estimatedUsd: 0.04
    });
    expect(costs.services.selectedBot.sandbox).toMatchObject({
      inputUnits: 300,
      outputUnits: 0,
      estimatedUsd: 0.006168
    });
  ...
  it("does not count a retried computer usage event twice", () => {
    const usage = {
      botId: "bot-a",
      estimatedUsd: 0.001,
      id: "computer:bot-a:0:1000",
      inputUnits: 1,
    ...
    expect(tasks.getCosts("bot-a", "task-a").services.selectedTask.sandbox).toMatchObject({
      estimatedUsd: 0.001,
      inputUnits: 1
    });
  });

...
describe("new teammate chat", () => {
  ...
  it("creates and selects an empty teammate before its first message", async () => {
    const current = { id: "bot-current", name: "Current" };
    const created = { id: "bot-new", name: "Milo" };
    const initial = {
      archivedBots: [],
      bots: [current],
      realtime: { url: null },
    ...
    const selected = { ...initial, bots: [created, current], selectedBot: created };
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(jsonResponse(initial))
      .mockResolvedValueOnce(jsonResponse({ teammate: created }, 201))
      .mockResolvedValueOnce(jsonResponse(selected));
    vi.stubGlobal("fetch", fetchMock);
    const view = await renderComponent(createElement(WorkspaceHarness));

    await controller().beginNewTeammate();
    await interact();
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        body: JSON.stringify({ brief: "Start a conversation.", conversation: true }),
        method: "POST"
      })
    );
...
describe("teammate model", () => {
  ...
  it("saves and reloads a finite step limit", async () => {
    const bot = {
      id: "bot-1",
      maxSteps: null,
      name: "Teammate",
    ...
    const updatedBot = { ...bot, maxSteps: 32 };
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(jsonResponse(snapshot))
      .mockResolvedValueOnce(jsonResponse({ teammate: updatedBot }))
    ...
    await controller().setMaxSteps(32);
    await interact();
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      ...
      "/api/bots/bot-1",
      expect.objectContaining({
        body: JSON.stringify({ maxSteps: 32 }),
        method: "PATCH"
      })
    );
    expect(controller().selectedBot?.maxSteps).toBe(32);
    await view.unmount();
  });

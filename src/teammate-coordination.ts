...
export abstract class TeammateCoordinationRuntime extends TeammateProductRuntime {
  ...
  protected override productTools(): ToolSet {
    return {
      ...
      coordinate: tool({
        description:
          "Coordinate one team task. Read settings for owner-controlled management, hiring, model and budget limits. Team lists available teammates. Start saves a goal and completion criteria. Assign bounded jobs using stable keys; approved managers can delegate smaller parts within the same task, at most two levels below its owner (owner, manager, specialist). Wait ends the turn and resumes you when results or reports arrive. Report milestones and blockers with the next step. Peers lists active specialists on this task. Ask sends a bounded question with a stable key; answer replies using its questionId. Use these for evidence already available, not new work or permissions. Answer incoming questions before asking another specialist. At most three questions per assignment, twenty per task, one outgoing question at a time, with a ten-minute timeout. Continue independent work or wait for an answer; do not poll. Check_in requests progress; redirect sends changed guidance at the next safe step. Review your direct reports and their evidence before finish, with a check for each criterion (criterion 0 for a delegated manager). One root budget, deadline and cancellation cover all work. Hire creates a teammate only when the owner has enabled it. Private permissions, memory, files, connections and computers stay separate.",

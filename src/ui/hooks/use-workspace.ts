...
export function useWorkspace(onSignedOut: () => void) {
  ...
  async function beginNewTeammate(): Promise<void> {
    if (creatingTeammateRef.current || sending) return;
    creatingTeammateRef.current = true;
    setSending(true);
    setError("");
    ...
    setMobileChatOpen(true);
    try {
      const createdTeammate = await createTeammate("Start a conversation.");
      ...
      creatingTeammateRef.current = false;
      setSending(false);
    }
  }
  ...
  async function send(prompt: string): Promise<boolean> {
    ...
    if (!value || sending || creatingTeammateRef.current) return false;
    setSending(true);
    setError("");
  ...
  async function setMaxSteps(maxSteps: number | null): Promise<void> {
    if (!selectedBot) return;
    setError("");
    try {
      await api(`/api/bots/${selectedBot.id}`, {
      ...
      setError(errorMessage(cause, "The step limit could not be changed"));
      throw cause;
    }
  }
  ...
  async function stopSelectedTask(): Promise<void> {
    ...
    setError("");
    try {
      await api(`/api/bots/${selectedBot.id}/task/stop`, { method: "POST" });
      await load(selectedBot.id);
    } catch (cause) {

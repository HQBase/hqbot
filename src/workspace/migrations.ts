...
function migrateEight(sql: Sql): void {
  if (isApplied(sql, 8)) return;
  sql`UPDATE tasks SET status = 'cancelled', error = NULL, updated_at = ${now()}
    WHERE status NOT IN ('cancelled', 'completed', 'failed')`;
  finish(sql, 8);
}

function migrateNine(sql: Sql): void {
  if (isApplied(sql, 9)) return;
  if (!hasColumn(sql, "bots", "max_steps")) {
    sql`ALTER TABLE bots ADD COLUMN max_steps INTEGER CHECK (max_steps IS NULL OR max_steps >= 1)`;
  }
  finish(sql, 9);
}

function migrateTen(sql: Sql): void {
  if (isApplied(sql, 10)) return;
  if (!hasColumn(sql, "tasks", "work_state")) {
    sql`ALTER TABLE tasks ADD COLUMN work_state TEXT`;
  }
  if (!hasColumn(sql, "tasks", "wake_at")) {
    sql`ALTER TABLE tasks ADD COLUMN wake_at TEXT`;
  }
  finish(sql, 10);
}

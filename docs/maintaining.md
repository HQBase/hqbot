...
For live proof, use structured browser controls with the Chrome window in the Linux desktop. Prove
that owner takeover pauses model browser control. Prove one managed checkpoint, stop, and restore.
Confirm that CPU, memory, disk, uptime, and estimated cost appear after real computer use. Then
connect a dedicated remote MCP server and complete one useful tool-backed task.

Record only safe IDs, times, the Git SHA, Worker version, source links, resource readings, and cost
estimate. Do not record credentials, prompts, browser contents, checkpoints, or tool results.
